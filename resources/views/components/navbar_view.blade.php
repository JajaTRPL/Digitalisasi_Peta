...
<!-- Navbar (unchanged) -->
<nav class="bg-white shadow-md sticky top-0 z-[9999999999]">
  <div class="container mx-auto px-4 py-4 flex justify-between items-center">
    <div class="flex items-center space-x-4">
      <img src="{{ asset('images/sleman-logo.png') }}" alt="Logo" class="h-12 w-12">
      <p class="text-[16px] font-semibold font-poppins">Daftar Tanah Peta</p>
    </div>
    <div id="navLinks" class="items-center flex space-x-4">
      ...
      <a id="viewPeta" href="{{route('ViewPeta')}}" class="text-[#666CFF] hover:text-black">Peta</a>
      <a id="manageAdmin" href="{{route('manageAdmin')}}" class="text-[#666CFF] hover:text-black hidden">Kelola Admin</a>
      <a id="manageGround" href="{{route('ManageGround')}}" class="text-[#666CFF] hover:text-black hidden">Kelola Tanah</a>
...
<script>
    ...
    if (token) {
        ...
        $.ajax({
            ...
            success: function(user) {
                ...
                // Set warna acak dan inisial
                const randomColor = colors[Math.floor(Math.random() * colors.length)];
                avatar.addClass(randomColor);
  ...
  // Fungsi untuk menandai link aktif
    function setActiveNavLink() {
      ...
      // Hapus semua class aktif terlebih dahulu
      $('#navLinks a').removeClass('text-black').addClass('text-[#666CFF]');
      
      // Cocokkan path dengan route yang ada
      if (currentPath.includes('/dashboard')) {
        $('#dashboard').removeClass('text-[#666CFF]').addClass('text-black');
      } else if (currentPath.includes('/ViewPeta')) {
        $('#viewPeta').removeClass('text-[#666CFF]').addClass('text-black');
      } else if (currentPath.includes('/admin')) {
        $('#manageAdmin').removeClass('text-[#666CFF]').addClass('text-black');
      } else if (currentPath.includes('/ManageGround')) {
        $('#manageGround').removeClass('text-[#666CFF]').addClass('text-black');
      }
    }
